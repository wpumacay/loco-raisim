...
#define NUM_MESHES      0 // meshes slow down quite a bit when using hfields (still, should use optimized convex-hull instead)
...
tysoc::TBody* createSimpleBody( const std::string& name, const std::string& type )
{
    ...
    else if ( type == "mesh" )
    {
        ...
        _collisionData.filename = std::string( TYSOC_PATH_MESHES_DIR ) + "cube.obj";

        _visualData.type = tysoc::eShapeType::MESH;
        _visualData.size = { 0.2, 0.2, 0.2 };
        _visualData.filename = std::string( TYSOC_PATH_MESHES_DIR ) + "cube.obj";
    }
    else 
    {
        return NULL;
